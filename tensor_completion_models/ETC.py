...
class ETC(nn.Module):
    ...
    def __init__(self, 
        ...
        self.special_agg = ['xgb', 'svm', 'rf', 'dt', 'knn']
        ...
        while (cond):
            ...
            for i, (train_index, test_index) in enumerate(kf.split(self.sparse_tensor.indices().t())):
                ...
                use_batch_size, use_lr, use_wd = batch_size, lr, wd

                if model_type == 'NeAT':
                    use_batch_size, use_lr, use_wd = 32, 1e-2, 1e-3

                new_model = train_tensor_completion(model_type = model_type,
                                                    sparse_tensor = self.sparse_tensor,
                                                    rank = int(rank),
                                                    non_smooth_modes = cpd_s_non_smooth_modes,
    ...
    def get_individual_predictions(self, idxs):
    ...
    def train_agg_model(self, idxs, targets):
        self.agg_model.fit(self.get_individual_predictions(idxs), targets)

    def forward(self, idxs):
            
        predictions = self.get_individual_predictions(idxs)
            
        if (self.agg_model is not None):
            return torch.tensor(self.agg_model.predict(predictions))
