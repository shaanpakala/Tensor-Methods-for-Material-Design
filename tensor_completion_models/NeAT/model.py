...
# Learing rate (0.001) # sparser, smaller
# Weight decay (0.0001)
# Rank size # bigger, better
# Dropout ratio1,2 # starts from the smaller ratio
# layer dims (list) #small mlps fine
...
class MLP(nn.Module):
    def __init__(self, dims, act='ReLU'):
        ...
        for i in range(0, len(dims)-2):
            ...
            if ''!=act: layers.append(getattr(nn, 'ReLU')())
...
class NeAT(nn.Module):
    def __init__(self, cfg):
        ...
        # [#number of modes, dimension, 1] works well with shallow layers (2, 3), small dimension (8, 16, 32)
        # [3, 16, 1]
    ...
    def predict(self, idxs):
        return self.cfg.unnorm(self(idxs))
