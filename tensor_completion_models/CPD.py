...
from tensor_completion_models.codes_costco_tucker.costco import *
from tensor_completion_models.codes_costco_tucker.read import *
from tensor_completion_models.codes_costco_tucker.utils import *
...
class CPD(nn.Module):
    ...
    def recon(self, idxs):
        ...
        # facs = [F.softplus(self.embeds[m](idxs[:, m]).unsqueeze(-1)) for m in range(self.nmode)]
    ...
    def get_factors(self):
        idxs = torch.cartesian_prod(*[torch.arange(s) for s in self.sizes])
        facs = [F.softplus(self.embeds[m]) for m in range(self.nmode)]
        return facs